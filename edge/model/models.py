...
class task:
    ...
    def __init__(self, requestdeviceid, applicationid, offloadingpolicyid, taskid, operationid, inputdata, formertasklist,
    ...
    def initfromdict(cls, taskdict):
        tmptask = task(taskdict['requestdeviceid'], taskdict['applicationid'], taskdict['offloadingpolicyid'], taskdict['taskid'],
...
class offloadingPolicy:
    ...
    def tostring(self):
        import json
        tmpdict = self.todict()
        return json.dumps(tmpdict, cls=MyEncoder, ensure_ascii=True).encode()


class application:

    def __init__(self, requestdeviceid, applicationid, taskidlist, formertasklist, nexttasklist, operationlist):
        self.requestdeviceid = requestdeviceid
        self.applicationid = applicationid
    ...
    def initfromdict(cls, applicationdict):
        tmpapplication = application(applicationdict['requestdeviceid'], applicationdict['applicationid'], applicationdict['taskidlist'],
    ...
    def initfromString(cls, applicationstringlines):
        ...
        for line in applicationstringlines:
            taskidlist.append(int(line.split()[2]))
            formertasklist.append([int(tmp) for tmp in line.split()[3].split(',')])
            nexttasklist.append([int(tmp) for tmp in line.split()[4].split(',')])
            operationidlist.append(int(line.split()[5]))
        return application(requestdeviceid, applicationid, taskidlist, formertasklist,
...
class networkinfo:
    ...
    def initfromString(cls, networkinfoString):
        ...
        # print("When init from String the networkinfo, the len of the content is:", len(content), content)
