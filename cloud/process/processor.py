...
def produce_newtask(thistaskid, thistimecostlist, newtaskid,outputdata, requestdeviceid, applicatonid, offloadingpolicyid):
    ...
    for i in range(len(tmptaskidlist)):
        # print("Type of the tmptaskidlist is{0} Type of the tmptaskid is{1}".format(type(tmptaskidlist[i]), type(tmptaskid)))
    ...
    tmptask = task(tmprequestdeviceid, tmpapplicationid, tmpoffloadingpolicyid, tmptaskid, tmpoperationid, tmpinputdata,
